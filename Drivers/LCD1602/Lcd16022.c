...
#define GPIO_EN_PIN   GPIO_Pin_6            //  使能端口号
...
#define GPIO_RS_PIN   GPIO_Pin_7            //  数据/命令端口号
...
#define GPIO_DATA_4_PIN   GPIO_Pin_15   //  数据线4_端口号
#define GPIO_DATA_5       GPIOD         //  数据线5_端口组
#define GPIO_DATA_5_PIN   GPIO_Pin_14   //  数据线5_端口号
#define GPIO_DATA_6       GPIOD         //  数据线6_端口组
#define GPIO_DATA_6_PIN   GPIO_Pin_13   //  数据线6_端口号
#define GPIO_DATA_7       GPIOD         //  数据线7_端口组
#define GPIO_DATA_7_PIN   GPIO_Pin_12   //  数据线7_端口号
...
void Lcd_GPIO_init(void)
{
    ...
    /********Data端口设置*************/
     ...
     GPIO_InitStructure.GPIO_Pin  = GPIO_DATA_4_PIN|GPIO_DATA_5_PIN|GPIO_DATA_6_PIN|GPIO_DATA_7_PIN; //  DB8~DB15
     ...
     GPIO_InitStructure.GPIO_Pin =  GPIO_EN_PIN|LCD_BL_PIN;     // 使能端口
     GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;  //标准输出模式
     GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; //50M时钟速度
     GPIO_Init(GPIO_EN, &GPIO_InitStructure);
	   GPIO_SetBits(GPIO_EN,LCD_BL_PIN);
	 /********读/写端口设置**********/
...
{  
	...
	for(i=0;i<2;i++)
	{
			...
			GPIO_ResetBits(GPIO_DATA_4, GPIO_DATA_5_PIN);  //DB5
			GPIO_ResetBits(GPIO_DATA_4, GPIO_DATA_6_PIN);  //DB6
			GPIO_ResetBits(GPIO_DATA_4, GPIO_DATA_7_PIN);  //DB7
			if(x&0X10)GPIO_SetBits(GPIO_DATA_4, GPIO_DATA_4_PIN);//DB4
			if(x&0X20)GPIO_SetBits(GPIO_DATA_4, GPIO_DATA_5_PIN);//DB5
			if(x&0X40)GPIO_SetBits(GPIO_DATA_4, GPIO_DATA_6_PIN);//DB6
			if(x&0X80)GPIO_SetBits(GPIO_DATA_4, GPIO_DATA_7_PIN);//DB7
			Lcd_En_Toggle();  //发使能脉冲
			//delay_us(1);   //延时100us
		  Lcd_Busy(); //测忙
			x<<=4;
